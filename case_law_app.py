...
def search_scrape(keyword, pagenum=0):
    ...
    cases = []
    try:
        st.info(f"Fetching page {pagenum}...")
        ...
        st.success(f"Page {pagenum} fetched, {len(cases)} cases found.")
        return cases
...
if keyword:
    ...
    all_cases = search_scrape(keyword)
    ...
    if not all_cases:
        ...
        if keyword_broad != keyword:
            ...
            all_cases = search_scrape(keyword_broad)

    if not all_cases:
        st.info("No results found.")
    else:
        st.success(f"✅ {len(all_cases)} case(s) found")
        ...
        # Download buttons
        st.download_button("📄 PDF", export_pdf(all_cases), "case_results.pdf", "application/pdf")
        st.download_button("📝 DOCX", export_docx(all_cases), "case_results.docx",
                           "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
        st.download_button("📊 Excel", export_excel(all_cases), "case_results.xlsx",
